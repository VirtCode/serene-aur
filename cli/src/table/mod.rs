...
pub fn table<const COUNT: usize>(columns: [Column; COUNT], rows: Vec<[ColoredString; COUNT]>, sep: &str) {
    ...
    let width = cmp::min(
        terminal_size().map(|(Width(w), _)|  w as usize).unwrap_or(usize::MAX) - (COUNT - 1) * sep.len(), // terminal size
    ...
    let width = data.into_iter().map(|(column, _, max, avg, w)| {
    ...
    // header
    let header = width.iter()
        .map(|(column, width)| format!("{:<1$.1$}", column.header, width))
        .intersperse(sep.to_string()).collect::<String>();

    println!("{}", header.italic());

    // body
    for row in rows {
        let row = row.iter().zip(width.iter()).map(|(s, (column, width))| {
            match (column.centered, column.ellipse, s.len() > *width) {
