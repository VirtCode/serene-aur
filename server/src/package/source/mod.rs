...
#[async_trait]
pub trait SourceImpl: Sync + Send {
    ...
    async fn update(&self, folder: &Path) -> anyhow::Result<()>;
    ...
    async fn load_build_files(
        ...
        archive.append_directory(folder, Path::new(""))
    }
}
...
impl Source {
    async fn update(
        ...
        if self.inner.get_state() != before
            && (self.inner.get_srcinfo(folder).await?.is_none() || self.srcinfo_override)
        {
            self.srcinfo = Some(
                srcinfo_generator
                    .lock()
                    .await
                    .generate_srcinfo(&self.inner.get_pkgbuild(folder).await?)
                    .await
