...
impl AurResolver {
    ...
    /// an instance should not be kept around for long, and should only be used for one "occcasion"
    /// for packages in future, to-be-built srcinfo will be used for resolving
    pub async fn start(db: &Database, future: &Vec<Package>) -> Result<Self> {
        let added = Package::find_all(db).await?.into_iter()
    ...
    /// creates a resolver
    /// takes a vec of packages for which the to-be-built srcinfo is used instead
    pub fn create_resolver(&mut self) -> Resolver {
        let own = PkgbuildRepo {
            name: "serene",
            pkgs: self.added.iter().collect()
        };
