...
pub fn initialize_alpm() -> Result<Alpm> {
    ...
    if !path.exists() {
        fs::create_dir(&path)
            .context("failed to create folder for sync libalpm")?;
    }
    ...
    for db in STOCK_DATABASES {
        let handle = alpm.register_syncdb_mut(db, SigLevel::NONE) // we don't care whether our package index can be trusted
            .with_context(|| format!("failed to add database '{db}' to libalpm"))?;

        handle.add_server(get_server_for(db))
...
pub fn synchronize_alpm(alpm: &mut Alpm) -> Result<()> {
    ...
    alpm.syncdbs_mut().update(false)
        .context("failed to synchronize databases with libalpm")?;
